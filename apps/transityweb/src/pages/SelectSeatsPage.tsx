...
export default function SelectSeatsPage({ tripId, originStopId, destStopId, originSeq, destSeq, passengers, tripLabel, fare }: Props) {
  ...
  const maxRow = layout?.seatMap ? Math.max(...layout.seatMap.map((s) => s.row)) : 0;
  const maxCol = layout?.seatMap ? Math.max(...layout.seatMap.map((s) => s.col)) : 0;

  type SeatItem = NonNullable<typeof layout>['seatMap'][number];
  const seatGrid: (SeatItem | null)[][] = [];
  if (layout?.seatMap) {
    for (let r = 0; r <= maxRow; r++) {
      seatGrid[r] = [];
      for (let c = 0; c <= maxCol; c++) {
        seatGrid[r][c] = layout.seatMap.find((s) => s.row === r && s.col === c) || null;
      }
    }
  }
      ...
      <div className="bg-teal-900 px-4 pt-3 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-teal-300 text-[12px] mt-0.5">{tripLabel}</p>
          </div>
        ...
        {layout && (
          <div className="bg-white rounded-2xl shadow-soft p-5 anim-slide-up">
            <div className="flex items-center justify-center gap-5 mb-5 text-[11px] font-semibold text-slate-400">
              <span className="flex items-center gap-1.5"><span className="w-5 h-5 rounded-lg bg-slate-100 border border-slate-200" /> Tersedia</span>
