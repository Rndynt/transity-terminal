      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        {(step === 1 || step === 2) && (
          ...
          <div className="md:hidden flex-shrink-0 bg-white border-b border-gray-200">
            <div className="flex">
              <button
                onClick={() => { setMobilePanel('left'); if (step === 2) setStep(1); }}
                ...
                data-testid="mobile-tab-data"
              >
                Data Kiriman
              </button>
              <button
                onClick={() => { setMobilePanel('right'); if (step === 1 && canProceedStep1) { setTariffCache({}); setSelectedTripKey(''); setSelectedTripDate(''); setTripsSearched(true); setStep(2); setTimeout(() => { refetchAllTrips(); }, 50); } }}
                className={`flex-1 py-2.5 text-xs font-semibold text-center transition-colors ${
                  mobilePanel === 'right'
                    ? 'text-amber-700 border-b-2 border-amber-600 bg-amber-50/50'
                    : 'text-gray-400 hover:text-gray-600'
                }`}
                data-testid="mobile-tab-jadwal"
              >
                Jadwal & Harga
              </button>
            </div>
                  ...
                  <div key={group.date} data-testid={`trip-group-${group.date}`}>
                    ...
                    <div className="space-y-2 mb-4" data-testid="trip-list">
                      {group.trips.map((trip, idx) => {
                        ...
                        const tripIdentifier = trip.tripId || trip.baseId;
                        const isSelected = selectedTripKey === tripIdentifier && selectedTripDate === trip.date;
                        const tariff = trip.tariff;
                        return (
                          <button
                            key={tripKey + idx}
                            onClick={() => {
                              setSelectedTripKey(tripIdentifier ?? '');
