...
export default function CargoTypesPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => cargoTypesApi.create(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string; data: any }) => cargoTypesApi.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/cargo-types'] });
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden" data-testid="cargo-types-page">
