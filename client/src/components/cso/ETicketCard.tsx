import { Calendar, Bus, Store, Users, CreditCard, QrCode } from 'lucide-react';
...
export default function ETicketCard({
  ...
  const resolvedTitle = title ?? (isPaid ? 'E-TICKET' : 'INVOICE');
  const resolvedHeaderColor = headerColor ?? (isPaid
  ...
  return (
    <div className="w-full bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm print-section" data-testid="eticket-card">
      <div className={`px-5 py-3 flex items-center justify-between ${resolvedHeaderColor}`}>
      ...
      <div className="p-5">
        ...
        <div className="grid grid-cols-2 gap-y-3 gap-x-2 text-sm">
          <InfoCell icon={Calendar} label="Tanggal" value={formatDate(booking?.tripDetails?.serviceDate || booking?.departAt || booking?.createdAt)} />
          <InfoCell icon={Bus} label="Armada" value={booking?.vehicle?.plate || '-'} />
            ...
            <table className="w-full text-sm" data-testid="passenger-table">
              <thead>
                <tr className="text-[10px] text-gray-400 uppercase tracking-wider">
                  ...
                  {showFareColumn && <th className="text-right py-1 font-medium w-20">Tarif</th>}
                </tr>
              </thead>
