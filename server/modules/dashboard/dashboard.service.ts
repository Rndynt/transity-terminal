...
export class DashboardService {
  async getTodaySummary() {
    ...
    // 8 query independent — parallelize via Promise.all. Sebelumnya sequential
    // ~8 * RTT, sekarang ~max(query_durations). Pool size sudah di-tune
    // (PG_POOL_MAX default 20) supaya 8 koneksi simultan tidak saturate.
    const [
      tripsRaw,
      bookingsRaw,
      revenueRaw,
