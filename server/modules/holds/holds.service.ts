...
export class HoldsService {
  ...
  async createSeatHold(
    ...
    try {
      const existingHolds = await db.select()
        .from(seatInventory)
        .where(and(
      ...
      for (const row of existingHolds) {
        if (row.holdRef) {
          const [existingHold] = await db.select()
            .from(seatHolds)
            .where(eq(seatHolds.holdRef, row.holdRef))
            .limit(1);

          if (existingHold && new Date(existingHold.expiresAt) > new Date()) {
            if (existingHold.operatorId === owner.operatorId) {
