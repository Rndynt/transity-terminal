...
**Preferred fix for a missing column/table drift (seen repeatedly with `trip_patterns.allow_intra_city_booking`):** instead of `db:push`, hand-write a new numbered SQL file in `migrations/` (`ALTER TABLE ... ADD COLUMN IF NOT EXISTS ...`) and add a matching entry to `migrations/meta/_journal.json` (next `idx`, a `tag` matching the filename). The app's own `drizzle-orm` `migrate()` only reads the journal + `.sql` files at runtime — it does not need the `meta/*_snapshot.json` files — so this stays inside the project's existing migration bookkeeping and survives future boots without re-touching `db:push`.
</content>
