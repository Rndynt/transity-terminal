...
**Why:** Hit this on a fresh import — `0024_passenger_price_matrix.sql` existed but wasn't in `_journal.json`, so `trip_patterns.allow_intra_city_booking` and the `passenger_price_matrices`/`passenger_price_exceptions` tables (since renamed to `price_rules`/`price_rule_exceptions` as part of the pricing identity-swap — `0024` itself has since been deleted and folded into `0013_pricing.sql`; kept here as-is since this note describes the drift as it looked at the time it was hit) were missing after running the app's own migrator, causing seed script failures with "column does not exist". The `0024` file's own header comment says it's applied via `npm run db:push`, not the migrator — confirming `db:push` is the intended path for that change.
