...
function RouteTimeline({ stops, origin, destination, onOriginSelect, onDestinationSelect, onProceed }: {
  ...
  const stopMeta: Record<string, { time: string; dist?: string; dur?: string }> = {
    ...
    "Purwakarta": { time: "07:15", dist: "85 km", dur: "1j 15m" },
    "Bandung Terminal": { time: "09:30", dist: "45 km", dur: "2j 15m" },
    "Cirebon": { time: "09:00", dist: "130 km", dur: "3j" },
    "Pekalongan": { time: "11:30", dist: "95 km", dur: "2j 30m" },
              ...
              {!isLast && nextMeta?.dist && (
                <div className="flex items-center px-4 py-0">
                  ...
                  <div className="flex items-center gap-2 text-[10px] text-gray-400 -my-1">
                    <span className="flex items-center gap-1"><ArrowDown className="w-2.5 h-2.5" />{nextMeta.dist}</span>
                    <span className="text-gray-200">|</span>
                    <span className="flex items-center gap-1"><Clock className="w-2.5 h-2.5" />{nextMeta.dur}</span>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="bg-white border-2 border-blue-200 rounded-xl p-4 shadow-sm">
          <div className="flex items-center gap-4 mb-3">
            <div className="flex-1">
              ...
              <p className="text-[11px] text-gray-400 font-mono">{stopMeta[origin]?.time}</p>
            </div>

            <div className="flex flex-col items-center gap-1 px-3">
              <div className="flex items-center gap-1">
                <div className="w-2 h-2 rounded-full bg-emerald-400" />
                <div className="h-px w-8 bg-gray-200 relative">
...
type PassengerData = { seatNo: string; name: string; phone: string; id: string };

function PassengerFormPanel({ seats, price, onConfirm }: { seats: string[]; price: number; onConfirm?: () => void }) {
  ...
  useEffect(() => {
    setPassengers(prev => {
      ...
      return seats.map(s => existing.get(s) || { seatNo: s, name: "", phone: "", id: "" });
    });
  }, [seats]);
  ...
  const filledCount = passengers.filter(p => p.name.trim().length >= 3).length;
  ...
  const MAX_VISIBLE = 4;
  const formContainerMaxH = seats.length > MAX_VISIBLE
    ...
    <div className="h-full flex flex-col gap-0">
      <div className="flex items-center justify-between mb-3 flex-shrink-0">
        <div>
          ...
          <p className="text-[11px] text-gray-400">{filledCount}/{seats.length} terisi</p>
        </div>
        {seats.length > MAX_VISIBLE && (
      ...
      >
        {passengers.map((p, i) => {
          const nameKey = `name-${p.seatNo}`;
          const phoneKey = `phone-${p.seatNo}`;
          const nameError = getNameError(p.name, nameKey);
          const phoneError = getPhoneError(p.phone, phoneKey);
          const isFilled = p.name.trim().length >= 3;
          return (
            <div key={p.seatNo} className={`border rounded-xl p-3 transition-colors ${
      ...
      <div className="border-t border-gray-200 pt-3 mt-3 space-y-3 flex-shrink-0">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] text-gray-400">{seats.length} penumpang</span>
        </div>

        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="px-3 py-2 flex items-center justify-between border-b border-gray-100">
            <span className="text-xs text-gray-500">Harga per kursi</span>
            <span className="text-xs font-semibold text-gray-700 font-mono">{fmt(price)}</span>
          </div>
          <div className="px-3 py-2 flex items-center justify-between border-b border-gray-100">
            <span className="text-xs text-gray-500">Jumlah kursi</span>
            <span className="text-xs font-semibold text-gray-700">{seats.length} kursi</span>
          </div>
              ...
              return (
                <button key={m.id} onClick={() => setMethod(m.id)} data-testid={`pay-${m.id}`}
                  className={`p-2 rounded-lg border text-center transition-all ${
                    method === m.id ? "bg-blue-50 border-blue-400 ring-1 ring-blue-200" : "bg-white border-gray-200 hover:border-gray-300"
                  }`}>
          ...
          <div className="flex gap-2 items-start">
            ...
            {cashReceived >= total && (
              <div className="flex-1 bg-emerald-50 border border-emerald-200 rounded-lg p-2 text-center">
                ...
                <span className="text-lg font-black text-emerald-600 font-mono">{fmt(change)}</span>
              </div>
            )}
          </div>
        )}
      ...
      <div className="flex gap-2 pt-3 mt-auto border-t border-gray-200 flex-shrink-0">
        ...
        <button data-testid="btn-pay-confirm" onClick={onConfirm}
            ...
            {phase === "book" && (
              <>
                ...
                <div className="flex-1 overflow-y-auto p-5 flex flex-col">
                  {seats.length > 0 ? (
                    <PassengerFormPanel seats={seats} price={selectedTrip?.price ?? 75000} onConfirm={() => setShowPrint(true)} />
                  ) : (
                    <div className="h-full flex flex-col items-center justify-center text-gray-300">
