...
**Current dev DB seed:** re-imported on 2026-07-11 — ran `npx tsx scripts/db-migrate.ts` then `npm run db:push` (schema had drifted: `trip_patterns.allow_intra_city_booking` and the `passenger_price_matrices`/`passenger_price_exceptions` tables — since renamed to `price_rules`/`price_rule_exceptions` as part of the pricing identity-swap; this note is kept as-is for historical accuracy about what the drift looked like at the time — exist in `shared/schema` and in migration `0024_passenger_price_matrix.sql` (since deleted, folded into `0013_pricing.sql`), but `0024` was never added to `migrations/meta/_journal.json`, so Drizzle's migrator silently skipped it — `db:push` is what actually applies it, matching the comment left in that SQL file). Seeded with `npx tsx server/seeds/index.ts nusa` (Nusa Shuttle dataset: Jakarta/Bandung/Semarang/Yogyakarta).

**Re-imported again on 2026-07-13:** `npm install`, then `npx tsx scripts/db-migrate.ts`, then `npx tsx server/seed.ts nusa`, then started the `Start application` workflow. Hit the same `trip_patterns.allow_intra_city_booking` drift as before, but this time fixed it without `db:push`: added `migrations/0024_trip_patterns_intra_city.sql` (`ALTER TABLE ... ADD COLUMN IF NOT EXISTS`) plus a matching entry in `migrations/meta/_journal.json`, so the app's own migrator picks it up on every boot going forward — no need to run `db:push` again for this column.
